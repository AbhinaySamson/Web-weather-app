...
navigator.geolocation.getCurrentPosition(async function (position) {
    try {
        ...
        // Use the first result and trim the name
        let loc = userdata[0].name.split(" ")[0]; // Take only the first part of the name
        let country = userdata[0].country; // Get the country code
        console.log("Location found:", loc); // Log the location

        // Fetch weather details using city name
        let url = `https://api.openweathermap.org/data/2.5/forecast?&units=metric&`;
        let respond = await fetch(url + `q=${loc},${country}&` + `appid=${apiKey}`);
        let data = await respond.json();

        // Log the weather API response
        ...
        const forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${data.city.name},${country}&appid=${apiKey}&units=metric`;
